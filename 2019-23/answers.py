...
def solve(intcode):
    ...
    for i in range(50):
        ...
        if status == Computer.DONE:
            print("Computer {0} halted on startup.  Output {1}".format(i, computer.get_and_clear_output()))
    ...
    while True:
        for i in range(50):
            ...
            if computer2 is None:
                print("Error: sending message to halted computer: from {0} to {1}".format(i, dest))
            computer2.push_input(y_val)
            computer2.push_input(x_val)
            status = computer2.resume()
            if status == Computer.DONE:
                print("Computer {0} halted after message ({1},{2}) from {3}".format(dest, x_val, y_val, i))
                print("Computer {0} output = {1}".format(dest, computer.get_and_clear_output()))
                computers[dest] = None
    return 0
