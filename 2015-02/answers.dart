...
class Package {
  ...
  int get smallest_side_perimeter =>
      min(2 * h + 2 * w, min(2 * h + 2 * l, 2 * w + 2 * l));
...
Stream<Package> packageStream() => stdin
    .transform(utf8.decoder)
    .transform(const LineSplitter())
    .transform<Package>(StreamTransformer.fromHandlers(
        handleData: (line, EventSink sink) => sink.add(Package(line))));
