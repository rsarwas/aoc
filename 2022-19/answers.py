...
def parse(lines):
    data = []
    for line in lines:
        line = line.strip().replace("Blueprint ","").replace(": Each ore robot costs ",",")
        line = line.replace(" ore. Each clay robot costs ",",").replace(" ore. Each obsidian robot costs ",",")
        line = line.replace(" clay. Each geode robot costs ",",").replace("ore and ",",").replace(" obsidian.","")
        items = line.split(",")
        blueprint = {
            'id':int(items[0]),
            'ore':int(items[1]),
...
def max_geodes_cracked(blueprint, time):
    ...
    print("time to first obs: ore required:", ore2+ore3,"clay required:", clay, "clay time:",clay_time + clay, "+buid", obs_time, "minutes")
    geo_time = 1 + max(ore1+ore2+ore3, obs_time + obs)
    print("time to first geo: ore required:", ore1+ore2+ore3,"obs required:", obs, "obs time:",obs_time + obs, "+buid", geo_time, "minutes")
