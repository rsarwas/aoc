...
class Life {
  ...
  func update() -> Bool {
    var didChange = false
    let row = [Cell](repeating: .floor, count: w)
    var newGrid = [[Cell]](repeating: row, count: h)
    for y in 0..<h {
      for x in 0..<w {
  ...
  func sparse(_ x: Int, _ y: Int, n: Int) -> Bool {
    ...
    return part1
      ? adjacentSeats(x, y).filter { $0 == .occupied }.count <= n
  ...
  func visibleSeats(_ x: Int, _ y: Int) -> [Cell] {
    ...
    while x + inc < w && y + inc < h {
      if grid[y + inc][x + inc] != .floor {
        cells.append(grid[y + inc][x + inc])
        break
      }
      inc += 1
    }
    ...
    inc = 1
    while 0 <= x - inc {
      if grid[y][x - inc] != .floor {
        cells.append(grid[y][x - inc])
        break
      }
      inc += 1
    }
    ...
    inc = 1
    while 0 <= y - inc {
      if grid[y - inc][x] != .floor {
        cells.append(grid[y - inc][x])
        break
      }
      inc += 1
    }
