...
class ImagePuzzle: CustomStringConvertible {
  ...
  func sort() {
    ...
    for (pieceId, piece) in pieces {
      for edgePair in piece.edgePairs {
        ...
        if edges[egde] != nil { edges[egde]!.insert(pieceId) } else { edges[edge]!.insert(pieceId) }
      }
    }
    ...
    for v in singleUseEdges.values {
      ...
      if c == nil { unusedEdges[id] = 1 } else { unusedEdges[id] = c! + 1 }
    }
  ...
  func solve() -> Bool {
    ...
    let searchTiles: Set<PieceId> = {
      ...
      if position.x == 0 {
        ...
        guard let topEdge = top(position) else {
        ...
        guard let leftEdge = left(position) else {
          print("error no left")
          return Set<PieceId>()
        }
        //print("left:\(leftEdge)")
        ...
        guard let topEdge = top(position), let leftEdge = left(position) else {
          print("error no left")
          return Set<PieceId>()
        }
        guard let e1 = edges[topEdge], let e2 = edges[leftEdge] else {
          print("error no upper")
          return Set<PieceId>()
        }
    ...
    for tileId in searchTiles {
      for orient in Orientation.allCases {
        if fits(piece: (tileId, orient), at: position) {
          ...
          switch pieceType(position) {
          case .corner:
            unplacedCorners.remove(tileId)
            break
          case .edge:
            unplacedEdges.remove(tileId)
            break
          case .middle:
            unplacedMiddles.remove(tileId)
            break
          }
          if solve() {
            return true
            ...
            switch pieceType(position) {
            case .corner:
              unplacedCorners.insert(tileId)
              break
            case .edge:
              unplacedEdges.insert(tileId)
              break
            case .middle:
              unplacedMiddles.insert(tileId)
              break
            }
            board[position] = nil
          }
  ...
  func pieceType(_ position: Coord2) -> PieceType {
    if position.x == 0 && position.y == 0 || position.x == 0 && position.y == size - 1
...
struct Piece: CustomStringConvertible {
  ...
  init?(id: Int, data: [String]) {
    ...
    guard let b = Int(String(data[size - 1].map { $0 == "#" ? "1" : "0" }), radix: 2) else {
      return nil
    }
    ...
    guard let r = Int(String(rightChars.map { $0 == "#" ? "1" : "0" }), radix: 2) else {
      return nil
    }
    top = t
    bottom = b
  ...
  var edgePairs: [Coord2] {
    [
      Coord2(x: top, y: pot), Coord2(x: bottom, y: mottob), Coord2(x: left, y: tfel),
      Coord2(x: right, y: thgir),
    ]
  }
  ...
  static func reverse(_ x: Int, size: Int) -> Int {
    ...
    let zeros = String([Character](repeating: "0", count: size - bin.count))
